...
typedef struct _bt_hfp_hf_connection {
    bt_address_t addr;
    struct bt_conn* conn;
    struct bt_conn* sco_conn; /* SCO/eSCO connection when audio is up */
    bt_list_t* calls;
    struct bt_hfp_hf* hf;
...
static inline __attribute__((unused)) bt_hfp_hf_connection_t* find_connection_by_conn(struct bt_conn* conn)
{
    return (bt_hfp_hf_connection_t*)bt_list_find(g_sal_hf_conn_list, sal_conn_addr_cmp, conn);
}
    ...
    hfp_hf_call_state_t state)
{
    ...
    if (state == HFP_HF_CALL_STATE_DISCONNECTED && sal_conn && sal_conn->calls) {
...
static void zblue_on_outgoing_call(struct bt_hfp_hf* hf, struct bt_hfp_hf_call* call)
{
    ...
    bt_hfp_hf_call_info_t* sal_call = ensure_call(sal_conn, call);
    if (!sal_call) {
...
static void zblue_on_call_terminate(struct bt_hfp_hf_call* call)
{
    ...
    set_call_state(sal_conn, sal_call, HFP_HF_CALL_STATE_DISCONNECTED);
}
...
static void zblue_on_current_call(struct bt_hfp_hf* hf, struct bt_hfp_hf_current_call* call)
{
    ...
    if (!call) {
        ...
        hfp_hf_on_current_call_response(
            &bd_addr,
            0,
            0,
            0,
...
bt_status_t bt_sal_hfp_hf_init(uint32_t hf_features, uint8_t max_connection)
{
    ...
    SAL_CHECK_RET(Z_API(bt_hfp_hf_register)(&hf_callbacks), 0);
...
bt_status_t bt_sal_hfp_hf_reject_call(bt_address_t* addr)
{
    ...
    bt_hfp_hf_call_info_t* incoming_call = find_call_in_state(sal_conn, HFP_HF_CALL_STATE_INCOMING);
    if (!incoming_call) {
...
bt_status_t bt_sal_hfp_hf_hangup_call(bt_address_t* addr)
{
    ...
    bt_hfp_hf_call_info_t* target = find_call_in_state(sal_conn, HFP_HF_CALL_STATE_ACTIVE);
    if (!target) {
        target = find_call_in_state(sal_conn, HFP_HF_CALL_STATE_DIALING);
    }
    if (!target) {
        target = find_call_in_state(sal_conn, HFP_HF_CALL_STATE_ALERTING);
    }
...
bt_status_t bt_sal_hfp_hf_call_control(bt_address_t* addr, hfp_call_control_t chld, uint32_t index)
{
    ...
    case HFP_HF_CALL_CONTROL_CHLD_0: {
        bt_hfp_hf_call_info_t* waiting = find_call_in_state(sal_conn, HFP_HF_CALL_STATE_WAITING);
        if (waiting) {
            ret = Z_API(bt_hfp_hf_set_udub)(sal_conn->hf);
        } else {
            bt_hfp_hf_call_info_t* held = find_call_in_state(sal_conn, HFP_HF_CALL_STATE_HELD);
            if (held) {
...
bt_status_t bt_sal_hfp_hf_send_dtmf(bt_address_t* addr, char dtmf)
{
    ...
    bt_hfp_hf_call_info_t* active = find_call_in_state(sal_conn, HFP_HF_CALL_STATE_ACTIVE);
    if (!active) {
